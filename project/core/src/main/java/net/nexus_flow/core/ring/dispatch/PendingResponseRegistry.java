...
public final class PendingResponseRegistry implements AutoCloseable {
    ...
    private final ConcurrentHashMap<DispatchCorrelationId, Entry>       pendingResponseIndex      = new ConcurrentHashMap<>();
    private final ConcurrentHashMap<PeerId, Set<DispatchCorrelationId>> correlationIdsByPeerIndex = new ConcurrentHashMap<>();
    ...
    public CompletableFuture<DispatchResponseEnvelope> register(
        ...
        Entry                                       prior       = pendingResponseIndex.putIfAbsent(id, entry);
        if (prior != null) {
            inFlight.decrementAndGet();
            timeoutTask.cancel(false);
            throw new IllegalStateException("correlation id collision: " + id);
        }
        correlationIdsByPeerIndex.computeIfAbsent(peer, _ -> ConcurrentHashMap.newKeySet()).add(id);
        return future;
    }
    ...
    public boolean complete(DispatchCorrelationId id, DispatchResponseEnvelope response) {
        ...
        Entry entry = pendingResponseIndex.remove(id);
        if (entry == null) {
            return false;
        }
    ...
    public int cancelAllForPeer(PeerId peer, Throwable cause) {
        Objects.requireNonNull(peer, "peer");
        Objects.requireNonNull(cause, "cause");
        Set<DispatchCorrelationId> ids = correlationIdsByPeerIndex.remove(peer);
    ...
    public int inFlightFor(PeerId peer) {
        Set<DispatchCorrelationId> ids = correlationIdsByPeerIndex.get(peer);
    ...
    private void removeFromPeerIndex(PeerId peer, DispatchCorrelationId id) {
        ...
        ids.remove(id);
        if (ids.isEmpty()) {
            correlationIdsByPeerIndex.remove(peer, ids);
        }
    }

    @Override
    public void close() {
        Iterator<Map.Entry<DispatchCorrelationId, Entry>> it = pendingResponseIndex.entrySet().iterator();
        while (it.hasNext()) {
        ...
        correlationIdsByPeerIndex.clear();
        inFlight.set(0);
    }
}
