...
public final class RingConnectionRegistry {
    ...
    /**
     * Live view of the currently-registered peer ids. Returns the underlying
     * {@link java.util.concurrent.ConcurrentHashMap}'s weakly-consistent key set —
     * iteration is safe under concurrent registration but reflects mutations that happen
     * during the walk. Use {@link #peerIds()} when an immutable snapshot is required.
     *
     * <p>Intended for hot internal callers (e.g. the heartbeat tick) that only read.
     *
     * @return live key set view; never {@code null}
     */
    public java.util.Set<PeerId> peerIdsView() {
        return byPeerId.keySet();
    }
