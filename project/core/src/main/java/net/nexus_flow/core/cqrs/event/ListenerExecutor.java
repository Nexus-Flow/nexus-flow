...
final class ListenerExecutor<E extends DomainEvent> {
    ...
    ListenerExecutor(
        ...
        // Unfair semaphore — barging beats FIFO ordering by ~10× under sustained dispatch
        // pressure. Per-listener concurrency is a CAPACITY gate, not an ordering primitive;
        // event ordering is guaranteed by the bus's sequential listener iteration (or by
        // partitionKey-aware fan-out), not by semaphore queue order.
    ...
    HandlerInvoker<DomainEvent, Void> toInvoker(HandlerInvoker<DomainEvent, Void> base) {
        return new HandlerInvoker<>() {
            ...
            public @Nullable Void invoke(DomainEvent event, ExecutionContext ctx) throws Throwable {
                ...
                // idempotencyKey != null implies deduplicator != null (it is only resolved when
                // deduplicator is wired), so the redundant null check on deduplicator drops out.
                if (idempotencyKey != null && !dedupClaimed) {
