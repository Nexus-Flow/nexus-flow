...
public final class InvocationContext {
    ...
    /**
     * thread-local view of the {@link InvocationContext} currently flowing through the dispatcher
     * onion, set by {@link SyncDispatcher#dispatchThrough}. The handler executor's back-pressure gate
     * uses this to stamp observability attributes (e.g. {@code handler.queueDepth}) on the same bag
     * the {@link DispatchInterceptor} chain reads from, without having to widen the {@code
     * CommandHandlerExecutor} surface.
     *
     * <p>The slot is only populated while a chain is actively executing; outside the dispatcher it
    ...
    public InvocationContext withStage(InvocationStage newStage) {
        ...
        return new InvocationContext(
                kind, payload, executionContext, errorPolicy, newStage, attributes);
    }
    ...
    public InvocationContext withExecutionContext(ExecutionContext newExecutionContext) {
        ...
        return new InvocationContext(
                kind, payload, newExecutionContext, errorPolicy, stage, attributes);
    }
}
