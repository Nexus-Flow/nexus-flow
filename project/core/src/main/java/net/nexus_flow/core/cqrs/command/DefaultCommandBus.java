...
non-sealed class DefaultCommandBus implements CommandBus {
    ...
    /**
     * Pre-resolved interceptor list — read once at construction. The runtime's interceptor
     * binding is immutable after startup (set via the builder before the runtime is
     * built), so caching here saves the per-dispatch {@code runtime.interceptors()}
     * virtual call. The cached reference is the same immutable list the runtime owns —
     * what {@code FlowRuntime#interceptors()} would return on every call.
     */
    private final List<net.nexus_flow.core.runtime.dispatch.DispatchInterceptor> cachedInterceptors;
    ...
    DefaultCommandBus(
        ...
        this.cachedInterceptors = runtime.interceptors();
    }
    ...
    public <T extends Record, R> DispatchResult<R> dispatchAndReturnResult(
        ...
        List<net.nexus_flow.core.runtime.dispatch.DispatchInterceptor> interceptors = cachedInterceptors;
    ...
    private static @Nullable String failureClassOf(DispatchResult<?> r) {
        return switch (r) {
            ...
            case DispatchResult.Failure<?> f                              -> f
                    .cause() == null ? null : net.nexus_flow.core.runtime.ClassNameCache.get(f.cause().getClass());
            case DispatchResult.PartialFailure<?> p                       ->
                 p.failures().isEmpty() ? null : net.nexus_flow.core.runtime.ClassNameCache.get(p.failures().getFirst().getClass());
        };
    }
