...
public final class FastUuid {
    ...
    public static UUID v4() {
        return v4(ThreadLocalRandom.current());
    }

    /**
     * Overload accepting a pre-fetched {@link ThreadLocalRandom} so callers that need
     * multiple v4 UUIDs in a row can batch the {@link ThreadLocalRandom#current()} lookup
     * once instead of paying for it on every call. {@code ExecutionContext.root()} stamps
     * three IDs ({@code MessageId}, {@code TraceId}, {@code CorrelationId}) per allocation;
     * batching here shaves the redundant {@code current()} calls off the hot path.
     *
     * @param tlr the {@link ThreadLocalRandom} to draw bits from; never {@code null}
     * @return a v4 UUID
     */
    public static UUID v4(ThreadLocalRandom tlr) {
