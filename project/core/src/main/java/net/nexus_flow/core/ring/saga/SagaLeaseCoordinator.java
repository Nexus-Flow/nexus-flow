...
public final class SagaLeaseCoordinator implements AutoCloseable {
    ...
    private void broadcastRenewals() {
        ...
        long    expiryEpochMs = newExpiry.toEpochMilli();
        // Materialise the renewals locally first so the broadcast iterates a stable list AND
        // each frame is encoded exactly once. The previous shape re-encoded a SagaStateEnvelope
        // for every owned lease inside the broadcast call, which is fine for tens of leases
        // but quadratic-ish at the thousands scale operators target.
        java.util.List<byte[]> encodedFrames = new java.util.ArrayList<>(ownedLeases.size());
        for (SagaLease lease : ownedLeases.values()) {
            ...
            encodedFrames.add(new SagaStateEnvelope(
                    renewed.sagaId(),
                    renewed.ownerPeerId(),
                    expiryEpochMs,
                    0L).encode());
        }
        for (byte[] body : encodedFrames) {
            broadcast(body, FrameType.SAGA_STATE);
        }
    }
