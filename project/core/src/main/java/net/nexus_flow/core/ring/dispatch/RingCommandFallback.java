 ...
 * Without this fallback, cross-pod dispatch would be a manual operation through
 * {@link RingDispatcher} directly — this adapter wires the gap as an opt-in at the call
 * site, leaving the default {@link CommandBus} dispatch semantics unchanged.
 *
...
public final class RingCommandFallback {
    ...
    private static <R> DispatchResult<R> mapToLocalResult(DispatchResponseEnvelope envelope) {
        return switch (envelope.outcome()) {
            ...
            case FAILURE, PARTIAL_FAILURE, FORBIDDEN, UNAVAILABLE -> {
                String message = "cross-pod dispatch returned " + envelope.outcome()
                        + " (" + envelope.errorCode() + ")"
                        + (envelope.reason().isEmpty() ? "" : ": " + envelope.reason());
                // The wire envelope sanitises the remote stack trace (audit security finding):
                // the cause cannot carry the remote exception, so we synthesise a local
                // cause that preserves the wire-level outcome and error code as a stable
                // string. The originator's local logs hold the rich diagnostic indexed by
                // correlation id.
                yield DispatchResult.failure(
                                             new FlowError.Technical(
                                                     message,
                                                     new CrossPodDispatchException(message),
                                                     ExecutionContext.root()));
            }
            case NOT_FOUND                                        -> {
                String message = "no peer advertises the handler: " + envelope.reason();
                yield DispatchResult.failure(
                                             new FlowError.Technical(
                                                     message,
                                                     new CrossPodDispatchException(message),
            ...
            case TIMEOUT                                          -> {
                String message = "cross-pod handler timed out: " + envelope.reason();
                yield DispatchResult.failure(
                                             new FlowError.Technical(
                                                     message,
                                                     new CrossPodDispatchException(message),
    ...
    /**
     * Synthetic cause used to fill {@link FlowError.Technical}'s non-null {@code cause}
     * constructor parameter for cross-pod outcomes whose remote diagnostic has been sanitised
     * out of the wire envelope (sanitised by design — internal stack traces never cross
     * the trust boundary). The message mirrors the wire
     * outcome; the stack trace is the LOCAL one at the point the response was mapped.
     */
    public static final class CrossPodDispatchException extends RuntimeException {
        private static final long serialVersionUID = 1L;

        CrossPodDispatchException(String message) {
            super(message);
        }
    }
