...
/**
 * Bundle of every per-connection timeout enforced by the transport. Centralises the budgets
 * that used to live across {@link RingAcceptorConfig} and {@link RingDialerConfig}, including
 * a handshake timeout that the connection state machine actually enforces — without it, a
 * peer that completes TCP but never sends a HELLO would hold a connection slot for the full
 * idle window (a DoS surface). {@code ConnectionTimeouts} is the single carrier that the
 * connection state machine consults to apply the right SO_TIMEOUT at every phase.
 *
