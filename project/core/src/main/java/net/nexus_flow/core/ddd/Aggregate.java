...
public abstract non-sealed class Aggregate implements AggregateRoot {
     ...
     *
     * <p>{@link java.util.concurrent.locks.ReentrantLock} (not intrinsic monitor) — accidental
     * concurrent recording from multiple dispatcher threads makes this a contended hot path on
     * a hot aggregate. JMH validates ReentrantLock ≈ 2.7× faster than {@code synchronized}
     * under 8-thread contention.
     */
    private transient java.util.concurrent.locks.ReentrantLock lifecycleMonitor =
    ...
    public final void recordEvent(DomainEvent domainEvent) {
        ...
        try {
            // Single instanceof + downcast — both the staleness guard and the sequence
            // assignment used to repeat the pattern-match, paying for two type checks per
            // recorded event when the value is the same `AbstractDomainEvent` reference.
            AbstractDomainEvent abs = (domainEvent instanceof AbstractDomainEvent a) ? a : null;
            if (abs != null && abs.getSequenceNumber() != AbstractDomainEvent.UNASSIGNED_SEQUENCE_NUMBER) {
    ...
    /**
     * Batch-replay entry point — acquires {@link #lifecycleMonitor} ONCE for the whole list
     * instead of N times. {@link net.nexus_flow.core.eventsourcing.AggregateRepository#load
     * load} uses this to hydrate large aggregates without paying the lock-acquire cost per
     * event (saves 20–50 ns × N events on hot replay paths). Per-event semantics are
     * identical to single-event {@link #replay}: sequence number is advanced monotonically,
     * {@link #apply} is invoked for each event in iteration order, and the surrounding lock
     * guarantees concurrent {@link #recordEvent} calls cannot interleave with the replay.
     *
     * @param events events to replay in stream order; never {@code null}; empty is a no-op
     * @throws NullPointerException if {@code events} or any element is {@code null}
     */
    public final void replayAll(List<DomainEvent> events) {
        ...
        try {
            for (DomainEvent event : events) {
                Objects.requireNonNull(event, "events must not contain null entries");
                if (event instanceof AbstractDomainEvent abs) {
                    long seq = abs.getSequenceNumber();
                    if (seq >= 0 && seq >= nextSequenceNumber) {
                        nextSequenceNumber = seq + 1;
    ...
    public final void markCommitted(long newVersion) {
        ...
        try {
            ...
            // Snapshot the events being committed BEFORE the local state reset so we can
            // remove ONLY this aggregate's event references from the ambient sink.
            // Previously {@link DomainEventContext#clearEvents()} blasted the entire sink,
            // which is wrong when a handler touches several aggregates in the same scope —
            // the sibling aggregates' events would vanish from the ambient sink and the
            // runtime's drain would skip them. Surgical removal keeps every non-committed
            // event in the sink so the drain still publishes them.
            List<DomainEvent> committedEvents = uncommittedEvents;
            committedVersion = newVersion;
            uncommittedCount = 0L;
            // ArrayList.clear() walks the backing array nulling every slot. For a freshly-
            // committed aggregate that drained N events, an O(1) capacity reset (re-allocate
            // a fresh empty ArrayList) avoids the linear nulling pass.
            uncommittedEvents = new ArrayList<>();
            if (!committedEvents.isEmpty()) {
                eventContext.removeRecordedEvents(committedEvents);
            }
        } finally {
            lifecycleMonitor.unlock();
        }
    ...
    public final List<DomainEvent> drainEvents() {
        ...
        try {
            ...
            uncommittedCount = 0L;
            // Surgical removal — the caller is taking ownership of THIS aggregate's events,
            // so the runtime's ambient drain must not re-emit them. We strip only the
            // identities we are draining; sibling aggregates' events remain in the sink for
            // the runtime to publish.
            if (!snapshot.isEmpty()) {
                eventContext.removeRecordedEvents(snapshot);
            }
            return snapshot;
        } finally {
            lifecycleMonitor.unlock();
    ...
    private void readObject(ObjectInputStream input) throws IOException, ClassNotFoundException {
        ...
        this.lifecycleMonitor  = new java.util.concurrent.locks.ReentrantLock();
