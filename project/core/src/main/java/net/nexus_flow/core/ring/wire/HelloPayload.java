...
public record HelloPayload(
    ...
    public byte[] encode() {
        ...
        // Deterministic order — sorted set so two encodes of the same logical payload produce
        // bytewise-identical output (useful for signature/audit purposes downstream).
        for (String tenant : new TreeSet<>(tenants)) {
    ...
    private static Map<String, TypeFingerprint> readFingerprintMap(ByteBuffer buf) {
        ...
        Map<String, TypeFingerprint> result = new LinkedHashMap<>(count * 2);
