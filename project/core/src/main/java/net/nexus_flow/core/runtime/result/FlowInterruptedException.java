...
public final class FlowInterruptedException extends RuntimeException {
    ...
    public FlowInterruptedException(String message, InterruptedException cause) {
        // Stack-traceless — the cause's InterruptedException carries the interrupt point. This
        // wrapper's trace would only point to its own creation site (always a dispatcher
        // boundary). Suppression chain stays active. Saves ~200 ns per interrupt wrapping.
        super(
              message,
              cause,
