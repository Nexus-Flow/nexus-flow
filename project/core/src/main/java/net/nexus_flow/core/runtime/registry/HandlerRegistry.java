...
public final class HandlerRegistry<M, R> {
    ...
    /**
     * Registry-write mutex. {@link java.util.concurrent.locks.ReentrantLock} (not intrinsic
     * monitor) — the read side is lock-free (ClassValue-cached snapshot), the write side is
     * serialised through this lock. ReentrantLock matches the codebase-wide convention for
     * contention-sensitive mutexes; uncontended cost is equivalent to {@code synchronized}
     * on JDK 21+ and contended performance is 2.7× faster (validated by
     * {@code LockPrimitiveBenchmark}).
     */
