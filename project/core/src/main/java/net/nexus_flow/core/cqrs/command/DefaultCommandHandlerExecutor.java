...
final class DefaultCommandHandlerExecutor<T extends Record, R, H extends CommandHandler<T, R, H>>
    ...
    /**
     * Pre-resolved outbox config cached once at construction. Reading
     * {@link FlowRuntime#outbox()} returns a freshly-allocated {@link java.util.Optional}
     * on every call; the value never changes after runtime startup so we resolve once and
     * keep the unwrapped reference (or {@code null} when no outbox is bound). Saves one
     * Optional allocation + one virtual call per command-handler drain — at 1 M
     * command/sec that is measurable on the hot path.
     */
    private final net.nexus_flow.core.outbox.@org.jspecify.annotations.Nullable OutboxConfig cachedOutboxConfig;
    private final boolean                                                                    cachedUseOutboxFanOut;
    /**
     * Pre-resolved recursive-drain depth ceiling — read once at construction. The
     * runtime's value is immutable after startup; caching avoids the per-drain virtual
     * call.
     */
    private final int                                                                        cachedEventDrainMaxDepth;
    ...
    DefaultCommandHandlerExecutor(
        ...
        // Resolve once: the runtime's outbox binding does not change after startup. The
        // Optional return shape of FlowRuntime.outbox() allocates per call; caching the
        // unwrapped value + the kill-switch flag here means the per-drain hot path reads
        // two final fields instead of paying for Optional + instanceof on every event
        // emission.
        this.cachedOutboxConfig       = runtime.outbox().orElse(null);
        this.cachedUseOutboxFanOut    = cachedOutboxConfig != null && cachedOutboxConfig.useOutboxFanOut();
        this.cachedEventDrainMaxDepth = runtime.eventDrainMaxDepth();
    ...
    public void execute(List<Command<T>> commands) {
        ...
        // Explicit ArrayList + sort beats Stream + forEachOrdered on this size class:
        // Stream pipelines allocate a Spliterator + intermediate sink; ArrayList + List.sort
        // works in-place on the backing array. Saves ~40–80 ns per batch dispatch (small
        // batches; larger batches amortise the difference but never reverse it).
        List<Command<T>> sortedCopy = new ArrayList<>(commands);
        sortedCopy.sort(Comparator.comparingInt(Command<T>::getPriority).reversed());
        for (Command<T> c : sortedCopy) {
            execute(c);
        }
    }
    ...
    public List<R> submitAndReturn(List<Command<T>> commands) {
        ...
        // Same shape as the void-path execute(List<>) — explicit ArrayList sort beats
        // Stream sorted+map+toList on small-to-medium batches.
        List<Command<T>> sortedCopy = new ArrayList<>(commands);
        sortedCopy.sort(Comparator.comparingInt(Command<T>::getPriority).reversed());
        List<R> results = new ArrayList<>(sortedCopy.size());
        for (Command<T> c : sortedCopy) {
            results.add(submitAndReturn(c));
        }
        return results;
    }
    ...
    private void failDoomed(CommandTask<T, R> task) {
        ...
        try {
            ...
            future.completeExceptionally(FlowCancellationException.CANCELLED);
     ...
     *
     * <p>A single command handler can produce events that, when dispatched inline, cause listeners
     * to record MORE events. Those secondary events live in the same {@link
     * net.nexus_flow.core.cqrs.event.DomainEventContext} the handler used; they were previously
     * lost because the drain ran exactly once. The loop now snapshots the context, clears it,
     * appends + dispatches, and re-checks — until the context is quiescent OR the configured
     * {@link FlowRuntime#eventDrainMaxDepth()} ceiling is crossed (in which case the {@link
     * EventDrainOverflowException} surfaces as a {@code FlowError.Technical} on the caller).
     *
     * <p>Each iteration:
     *
     * <ol>
     * <li>Snapshot the current events (defensive copy so the inline-dispatch step does not
     * mutate the list we're iterating).
     * <li>Clear the context BEFORE dispatch so listener-emitted events accumulate in a fresh
     * buffer and become the input to the next iteration.
     * <li>Hand the snapshot to {@link HandlerEventDrain#drain}: outbox append first (if
     * configured), then inline fan-out (unless the outbox kill-switch is on).
     * </ol>
     *
     * <p>The clear-before-dispatch order is load-bearing: without it, the snapshot we take next
     * iteration would still contain the events we just dispatched.
     */
    private void drainRecordedEvents() {
        int depth    = 0;
        int maxDepth = cachedEventDrainMaxDepth;
        while (eventContext.hasEventsRecorded()) {
            depth++;
            if (depth > maxDepth) {
                throw new EventDrainOverflowException(depth, maxDepth);
            }
            List<DomainEvent> events = List.copyOf(eventContext.getEvents());
            eventContext.clearEvents();
            HandlerEventDrain.drain(events, cachedOutboxConfig, cachedUseOutboxFanOut, eventBus, sagaEnabled.get());
        }
    }
