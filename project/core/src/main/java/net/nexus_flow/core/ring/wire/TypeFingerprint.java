...
public record TypeFingerprint(byte[] bytes) {
    ...
    /**
     * Non-defensive view of the fingerprint bytes — returns the internal 32-byte array
     * WITHOUT a copy. Callers MUST treat the array as immutable; mutating it corrupts every
     * future reader. Intended for the framework's hot HELLO-encode / -decode path where the
     * caller is known to not mutate.
     */
