...
public final class JavaSerializationOutboxPayloadCodec implements OutboxPayloadCodec {
    ...
    public DomainEvent decode(byte[] bytes, Class<?> payloadType) {
        ...
        try (ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
            ...
            // {@link ObjectInputStream}'s constructor reads the wire stream header (4 magic
            // bytes + version) only — no class instantiation happens until the first {@link
            // #readObject} call below. Setting the filter HERE is correct: the filter governs
            // class-resolution decisions, which only fire during readObject's
            // {@code resolveClass} hook. No deserialization-driven side effects occur in the
            // constructor itself; the timing is safe.
