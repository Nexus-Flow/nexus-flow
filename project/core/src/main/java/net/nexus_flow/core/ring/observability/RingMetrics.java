 ...
 * <h2>Why a facade</h2>
 *
 * The audit identified that the ring layer emitted zero metrics despite the framework
 * exposing the SPI. Instead of sprinkling raw {@code metrics.incrementCounter("foo", Map.of(...))}
 * calls throughout the ring code — where typos, label-cardinality bugs, and inconsistent
 * naming would creep in — every ring metric flows through this single class. Metric names are
 * frozen constants; tag values are filtered through the cardinality rules described below.
 *
...
public final class RingMetrics {
    ...
    private Map<String, String> baseTags() {
        return Map.of("localPeer", localPeerId);
    }
