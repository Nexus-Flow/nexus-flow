 ...
 *
 * A single {@code deadlineEpochMillis} would silently fail under clock skew — a receiver with
 * a clock 5 s ahead of the sender would treat every command as already expired. The receiver
 * converts the deadline to its OWN monotonic clock as soon as the frame is decoded: the
...
public record DispatchRequestEnvelope(
    ...
    /**
     * Absolute spec-level ceiling matching the wire format's uint32 length-prefix capacity.
     * This is NOT the operator knob — runtime per-connection caps are configured at the
     * wire layer via {@link net.nexus_flow.core.ring.transport.RingAcceptorConfig.Builder#maxBodyBytes(int)},
     * which the {@link net.nexus_flow.core.ring.wire.FrameDecoder} enforces BEFORE this
     * envelope is constructed. The check here is a defense-in-depth bound for callers that
     * build the envelope outside the wire path (tests, in-process integrations).
     */
