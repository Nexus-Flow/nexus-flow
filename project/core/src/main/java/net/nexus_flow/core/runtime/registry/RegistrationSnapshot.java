...
final class RegistrationSnapshot {
    ...
    private final Map<Class<?>, List<RegisteredHandler<?, ?>>> handlersByType;

    /** Monotonic counter used to assign {@code sequence} on next add. */
    private final int nextSequence;

    private RegistrationSnapshot(
            Map<Class<?>, List<RegisteredHandler<?, ?>>> handlersByType, int nextSequence) {
    ...
    <M, R> List<RegisteredHandler<M, R>> handlersFor(Class<?> type) {
        List<RegisteredHandler<?, ?>> raw = handlersByType.get(type);
        if (raw == null) {
    ...
    <M, R> RegistrationSnapshot withRegistration(
        ...
        Map<Class<?>, List<RegisteredHandler<?, ?>>> next     = new LinkedHashMap<>(handlersByType);
    ...
    RegistrationSnapshot withoutInvoker(Class<?> type, HandlerInvoker<?, ?> invoker) {
        List<RegisteredHandler<?, ?>> existing = handlersByType.get(type);
        if (existing == null || existing.isEmpty()) {
            return this;
        }
